  ...
  for(i in 1:N_til){ //for each pair
    ...
    //for(j in 1:no_observations){ // for each observation period
      ...
      if(Nm[i]==0){ // there is no event occured in this period
        ...
        integ = interevent[Nm[i]]*temp_lambda0;
        forward_termination[1] = log_sum_exp(forward[Nm[i]] + probs_1[Nm[i]]) - integ*(1+temp_c);
        forward_termination[2] = log_sum_exp(forward[Nm[i]] + probs_2[Nm[i]]) - integ;
